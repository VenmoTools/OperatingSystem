...
fn new_idt() -> InterruptDescriptorTable {
    ...
    println!("debug func {:#X?}", debug as u64);
    // 在IDT中为双重错误处理程序设置堆栈索引
    unsafe {
        idt.double_fault.set_handler_fn(double_fault).set_stack_index(gdt::DOUBLE_FAULT_LIST_INDEX as u16);
    }
